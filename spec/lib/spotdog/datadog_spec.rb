...
module Spotdog
  describe Datadog do
    ...
    describe ".post_prices" do
      before do
        allow_any_instance_of(described_class).to receive(:post_prices).and_return([])
      end

      it "should create new #{described_class} isntance and call #post_prices" do
        expect_any_instance_of(described_class).to receive(:post_prices)
        described_class.post_prices(api_key, spot_prices)
      end
    end
